 ...
 *
 */
$row_positions = []; //must be kept global
        ...
        if ($files['error'] != 0) {
            ...
            if (strlen($extension) < 1 || !in_array(strtoupper($extension), $allowed_extensions)) {
                ...
                if (!move_uploaded_file($files['tmp_name'], $path)) {
                    ...
                    $headerTags = explode(',', str_replace(' ', '', 'email, first_name, last_name, dob, gender, company, street_address, suburb, city, state, postcode, country, telephone, fax, newsletter, send_welcome, zone_id, customers_group_pricing'));

                    for ($i = 0, $n = count($lines), $foundHeader = false; $i < $n; $i++) {
                        if (strlen($lines[$i]) > 0) {
...
function validate_customer(&$inArray, $dateFormatName)
{
    ...
    $return = ['cInfo' => $cInfo, 'errors' => $errors];
    return $return;
}
...
function create_customer_drop_down()
{
    global $db;
    $customers = [];
    $customers[] = ['id' => '0', 'text' => TEXT_PLEASE_CHOOSE];
    ...
    while (!$customersRecords->EOF) {
        $customers[] = [
            'id' => $customersRecords->fields['customers_id'],
            'text' => $customersRecords->fields['customers_firstname'] . ' ' . $customersRecords->fields['customers_lastname'] . ' (' . $customersRecords->fields['customers_email_address'] . ')'
        ];
